...
export default function Dashboard() {
  ...
  const { stats: projectStats, fetchStats: fetchProjectStats } = useProjectStore()
    ...
    <div className="space-y-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip formatter={(value: number, name: string, props?: { payload?: { amount?: number } }) => [
                  `${value}份 (${formatCurrency(props?.payload?.amount || 0)})`,
                  name
                ]} />
              </PieChart>
            </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-400 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-white/70 text-sm">项目总金额</p>
              <p className="text-4xl font-bold mt-2">{formatCurrency(projectStats?.totalAmount || 0)}</p>
            </div>
            <div className="bg-white/10 p-4 rounded-xl">

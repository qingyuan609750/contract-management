...
const dataDir = process.env.RENDER_DISK_MOUNT_PATH || process.cwd()
...
export function initDb() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS project_permission (
      ...
      can_manage_milestones INTEGER DEFAULT 0,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (project_id) REFERENCES project(id) ON DELETE CASCADE,
      FOREIGN KEY (user_id) REFERENCES user(id) ON DELETE CASCADE,
    ...
    CREATE INDEX IF NOT EXISTS idx_project_partner_project ON project_partner(project_id);
    CREATE INDEX IF NOT EXISTS idx_project_member_project ON project_member(project_id);
    CREATE INDEX IF NOT EXISTS idx_project_permission_project ON project_permission(project_id);
...
function runMigrations() {
  const migrations = [
    {
      name: 'add_project_id_to_contract',
      sql: 'ALTER TABLE contract ADD COLUMN project_id INTEGER'
    },
    {
      name: 'add_project_id_index',
      sql: 'CREATE INDEX IF NOT EXISTS idx_contract_project ON contract(project_id)'
    }
  ]
